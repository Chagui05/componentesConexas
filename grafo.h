...
public:
    ...
    // Si no se le pone peso, se asume 0, input de NodoGrafo
    ...
    // INODO
    // Si no se le pone peso, se asume 0, input de INodo
    void addArc(INodo *pOrigen, INodo *pDestino)
    {
        this->addArc(pOrigen->getId(), pDestino->getId(), 0); // se llama a funcion por id, ya que es un INodo
    }
    // lo mismo que el anterior, pero con peso
    ...
    vector<INodo *> seccionConexa(INodo *pOrigen)
    {
        vector<INodo *> result;
        queue<NodoGrafo *> nodosProcesados;
        int visitados = 0;
